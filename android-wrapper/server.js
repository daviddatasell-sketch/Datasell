...
app.get('/api/profile/stats', requireAuth, async (req, res) => {
  try {
    ...
    const transactionsSnap = await admin.database().ref('transactions').orderByChild('userId').equalTo(uid).once('value');
    const transactions = transactionsSnap.val() || {};
    const totalOrders = Object.keys(transactions).length;
    const totalSpent = Object.values(transactions).reduce((sum, transaction) => sum + (transaction.amount || 0), 0);
    
    // Count successful orders (status = 'success' or 'completed')
    const successfulOrders = Object.values(transactions).filter(transaction => 
      transaction.status === 'success' || transaction.status === 'completed' || transaction.status === true
    ).length;
    ...
    console.log(`📊 Profile stats for ${uid}: ${totalOrders} orders, ${successfulOrders} successful, ₵${totalSpent} spent`);
