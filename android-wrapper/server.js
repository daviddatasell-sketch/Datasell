...
window.__APK_URL = ${JSON.stringify(apkUrl)};
...
app.post('/api/initialize-payment', requireAuth, async (req, res) => {
  try {
    ...
    // Calculate Paystack amount (add 4% fee)
    const paystackAmount = Math.ceil(amount * 100 * 1.04);
...
app.get('/api/verify-payment/:reference', requireAuth, async (req, res) => {
  try {
    ...
    if (result.data.status === 'success') {
      ...
      // Send wallet funding SMS to user (android-wrapper)
      try {
        const userData = userSnapshot.val() || {};
        ...
        console.error('Wallet funding SMS error (android-wrapper):', smsErr);
      }
...
// Helper function to check if error is specifically a stock out issue
function isStockOutError(hubnetData) {
  if (!hubnetData) return false;
  ...
  // Check for stock-specific error messages
  const stockErrorKeywords = [
    'out of stock', 'no stock', 'stock unavailable', 'not available',
    'product unavailable', 'service unavailable - no stock'
  ];
  
  return stockErrorKeywords.some(keyword => 
    code.includes(keyword) || 
    reason.includes(keyword) || 
    event.includes(keyword) ||
    fullResponse.includes(keyword)
...
app.post('/api/purchase-data', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (processedHubnetData.status === true && processedHubnetData.code === "0000") {
      ...
      const isStockOut = isStockOutError(processedHubnetData);
      console.log('🔍 Balance error check:', { isOutOfStock, isStockOut, processedHubnetData });
      ...
      if (network === 'at' || network === 'airteltigo') {
        ...
        if (isStockOut) {
          ...
          errorMessage = processedHubnetData.reason || processedHubnetData.message || 'Purchase failed';
        }
      } else {
        // For other networks, show balance issue if detected
        errorMessage = isOutOfStock ? 'Out of Stock - Please try again later' : (processedHubnetData.reason || processedHubnetData.message || 'Purchase failed');
      }
    ...
    if (error.response && error.response.data) {
      ...
      const isStockOut = isStockOutError(hubnetErrorData);
      console.log('🔍 Balance error check (from catch):', { isOutOfStock, isStockOut, hubnetErrorData });
      ...
      if (network === 'at' || network === 'airteltigo') {
        ...
        if (isStockOut) {
          ...
          errorMessage = hubnetErrorData.reason || hubnetErrorData.message || 'Purchase failed';
        }
      } else {
        // For other networks, show provider balance issue if detected
        errorMessage = isOutOfStock ? 'Out of Stock - Please try again later' : (hubnetErrorData.reason || hubnetErrorData.message || 'Purchase failed');
      }
