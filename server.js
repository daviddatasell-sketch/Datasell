...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    try {
      ...
      googleUserData = decodedToken;
      console.log('✅ Google token verified:', googleUserData.email);
    } catch (error) {
      console.error('❌ Google token verification failed:', error.message);
      return res.status(401).json({ success: false, error: 'Invalid Google token' });
    }

    const { email, name, picture, uid: googleUid } = googleUserData;
    ...
    if (!existingUser) {
      ...
      // Check phone validation is not needed for OAuth (no phone provided)
      ...
      try {
        ...
        console.log('✅ Created Firebase Auth user for Google OAuth:', uid);
      } catch (authError) {
        if (authError.code !== 'auth/uid-already-exists') {
          console.warn('⚠️ Firebase Auth creation warning:', authError.message);
        }
      }
      ...
      console.log('✅ New Google OAuth user created:', { uid, email });
      ...
      // Update existing user's last login and sync Google data
      await admin.database().ref('users/' + uid).update({
        lastLogin: new Date().toISOString(),
        authMethod: 'google',
        googleUid: googleUid,
        profilePicture: picture || existingUser.profilePicture || null
      });

      console.log('✅ Existing user logged in via Google OAuth:', uid);
    }
    ...
    req.session.user = {
      uid: uid,
      email: email,
      displayName: name || existingUser?.displayName || 'User',
      isAdmin: email === process.env.ADMIN_EMAIL
    };
    ...
    return req.session.save((err) => {
      ...
      console.log('✅ Google OAuth session created:', { uid, email });
