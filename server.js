...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    sameSite: 'lax',
    maxAge: 30 * 24 * 60 * 60 * 1000 // 30 days (was 30 minutes, now more practical)
  },
  name: 'datasell.sid',
...
// Maintenance page route (still available if needed)
...
// Helper function to check if DataMart error is due to provider balance or stock
...
function isStockOutError(datamartData) {
  if (!datamartData) return false;
